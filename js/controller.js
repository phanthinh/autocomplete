...
test.controller('MasonryController', function($scope, $http,$location,$rootScope,$timeout) {
	...
	$scope.callbackFunc_1 = function(){
		console.log("callback function 1");
	}
	
	$scope.callbackFunc_2 = function(){
...
test.controller('UploadController', function($scope, $http,$location,$rootScope,$timeout) {
	...
	$scope.onFileSelect = function($files) {
		...
		angular.forEach($files, function(value, key){
			...
			if ($scope.fileReaderSupported && value.type.indexOf('image') > -1) {
	...
	$scope.start = function (index){
		...
		var fd = new FormData();
		fd.append('file', $scope.selectedFiles[index]);
		
		$http.post(
			'http://msyapi.com/upload', 
			fd, {
	...
	$scope.uploadPhotos = function (){
		...
		angular.forEach($scope.selectedFiles, function(value, key){
			...
			$http.post(
				...
				console.log(data);
				if(key==$scope.selectedFiles.length-1)
					$scope.uploadStatus = false;
			}, function(evt) {
				// Math.min is to fix IE which reports 200% sometimes
				$scope.progress[key] = Math.min(100, parseInt(100.0 * evt.loaded / evt.total));
			});
		})
	}
});